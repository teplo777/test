...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const videoSources = {
    mp4: 'img/bg-video/videobg.mp4',
    webm: 'img/bg-video/videobg.webm' // Рекомендуется добавить WebM версию
  };

  // 1. Проверка поддержки форматов
  function supportsVideoType(type) {
    const video = document.createElement('video');
    return video.canPlayType(`video/${type}`) !== '';
  }

  // 2. Установка оптимального источника
  function setVideoSource() {
    const source = document.createElement('source');
    
    if (supportsVideoType('webm')) {
      source.src = videoSources.webm;
      source.type = 'video/webm; codecs="vp9,opus"';
    } else {
      source.src = videoSources.mp4;
  ...
  // 3. Управление воспроизведением с обработкой ошибок
  async function handleVideoPlay() {
    try {
      ...
      // Попытка после взаимодействия пользователя
  ...
  function showFallback() {
    ...
    fallback.style.backgroundImage = 'url(img/bg-video/fallback.jpg)';
